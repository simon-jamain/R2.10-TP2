...
def db_describe():
    """
    Retourne les statistiques descriptives d'une série stockée en MySQL.
    Paramètre URL : serie (ex. ?serie=serie_A)
    """
...
def db_correlation():
    """
    Calcule la corrélation de Pearson entre deux séries stockées en MySQL.
    Paramètres URL : serie_x, serie_y (ex. ?serie_x=serie_A&serie_y=serie_B)
    """
    ...
    try:
        ...
        # Aligner les longueurs si les séries ont des tailles différentes
        n    = min(len(x), len(y))
        x, y = x[:n], y[:n]
